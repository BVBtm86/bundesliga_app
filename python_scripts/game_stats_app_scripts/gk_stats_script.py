...
# ##### Gk Stats Analysis Page
def gk_page(data:pd.DataFrame,
            favourite_team:str,
            page_season:str,
            season_teams:list,
    ...
    player_analysis = PlayerStatsAnalysis(player_type='Gk')
    ...
    # #### Gk Statistics Type
    gk_stats_options = player_analysis.analysis_options
    ...
    if stats_type == "Season":
        ...
        filter_stat = st.sidebar.selectbox(label="Game Stat", 
                                            options=player_analysis.player_stats.stats_gk)
        ...
        season_gk_stats = player_analysis.player_season_stats(data=season_data, 
                                                              team=favourite_team, 
                                                              player=team_gk_analysis)
        ...
        with type_chart_col:
            fig_type_gk, period_venue, period_venue_1, period_venue_2, period_insights, \
                period_insights_1, period_insights_2 = player_analysis.player_season_filter(data_player_agg=season_gk_stats, 
                                                                                            player=team_gk_analysis, 
                                                                                            stat_name=filter_stat)
            if match_day_fig is not None:
        ...
        pos_stat = player_analysis.player_stats.stats_gk.index(filter_stat)
        st.dataframe(season_gk_stats.style.format({stat: '{:.2f}' for stat in player_analysis.season_filter.season_filter}).apply(lambda x: [
    ...
    # ##### Gk vs Gk Statistics
        ...
        team_gk_radar, opponent_gk_radar, stat_comparison, comparison_insight = player_analysis.player_stats_comparison(
            data=season_data, 
            team=favourite_team,
            team_player=team_gk_analysis,
            opponent=opponent_team,
            opponent_player=opponent_gk_analysis,
            filter_season=filter_season)
        
        # ##### Gk Team vs Opponent Gk Stats Type Comparison
    ...
    # ##### Gk Last 5 Seasons Statistics
        ...
        all_seasons_gk_data = GameStatsProcessing().filter_season_data(
            data=BundesligaGameData().retrieve_all_seasons_player_data(
                table=GameStatsConfiguration().get_tab_info().gk_stats_tab,
